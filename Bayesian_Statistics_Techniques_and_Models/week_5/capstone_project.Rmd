...
If we fit the linear regression to our potentially cointegrated pairs, we will will get the resuduals and we'll be able to run the cointegration tests, for example the Augmented Dickey-Fuller Test.
...
```{r residuals_plot}
breaks <- 100
hist(comb1$residuals, breaks=breaks, main = "DAX ~ SMI residuals")
hist(comb4$residuals, breaks=breaks, main = "SMI ~ FTSE residuals")
``` 
